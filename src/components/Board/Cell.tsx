...
export type CellProps = { value: number };

type TileStyle = { gradient: string; color: string; fontSize: number };
...
export const Cell: FC<CellProps> = ({ value }) => {
  if (value === 0) return <div className={`${styles.cell} ${styles.empty}`} />;

  const { gradient, color, fontSize } = TILES[value] ?? FALLBACK;
